...
import { LayoutDashboard, Ticket, User, LogOut, Menu, X, PoundSterling, ChevronRight } from 'lucide-react';
...
const links = [
  { href: '/client/dashboard', label: 'Dashboard',  icon: LayoutDashboard, color: 'text-teal-400'  },
  { href: '/client/tickets',   label: 'My Tickets',  icon: Ticket,          color: 'text-amber-400' },
  { href: '/client/payments',  label: 'Payments',    icon: PoundSterling,   color: 'text-green-400' },
  { href: '/client/profile',   label: 'My Profile',  icon: User,            color: 'text-blue-400'  },
];
...
export default function ClientSidebar() {
        ...
        return (
          <Link key={href} href={href} onClick={onNav}
            className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
              active ? 'text-white nav-active-glow' : 'text-slate-400 hover:text-slate-200'
            }`}
            style={active ? {
  ...
  const SidebarInner = ({ mobile = false }) => (
    <div className="flex flex-col h-full">
      <div className="px-5 pt-6 pb-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-xl flex-shrink-0"
            style={{ background: 'linear-gradient(135deg,#0d9488,#0f766e)', boxShadow: '0 0 16px rgba(13,148,136,0.4)' }}>
            🏡
          </div>
          <div>
            <p className="text-white font-bold text-[15px] leading-tight">SupportHome</p>
            <p className="text-teal-400/60 text-[11px] font-medium tracking-wide">TENANT PORTAL</p>
          </div>
        </div>
      </div>

      <div className="mx-4 mb-4 h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(45,212,191,0.15), transparent)' }}/>
      ...
      <div className="mx-4 mt-4 mb-3 h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(45,212,191,0.15), transparent)' }}/>
      ...
      <div className="px-3 pb-5">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl mb-1"
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white text-sm font-semibold truncate leading-tight">{user?.firstName} {user?.lastName}</p>
    ...
    <>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-30 flex">
          ...
          <div className="relative w-72 flex flex-col pt-14" style={{ background: SIDEBAR_BG }}>
            <SidebarInner mobile/>
          </div>
        </div>
      )}
